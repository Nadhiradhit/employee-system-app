        ...
        <div class="card bg-base-100 border border-base-content/5">
            <div class="card-body">
                <form action="{{ route('employees.update', $employee->user_id) }}" method="POST"
                    ...
                    <x-forms.input name="joining_date" label="Joining Date" type="date" required :value="old('joining_date', $employee->joining_date?->format('Y-m-d') ?? '')" />

                    <x-forms.select name="status" label="Status" :options="['active' => 'Active', 'inactive' => 'Inactive']" :selected="old('status', $employee->status)" required />
